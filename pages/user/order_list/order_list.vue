<template>
	<view class="bg-gray">
		<!-- 顶部导航 -->
		<view class="topTabBar flex justify-around">
			<view class="grid flex align-center justify-center"  v-for="(grid,tbIndex) in orderType" :key="tbIndex" @tap="showType(tbIndex)">
				<view class="flex align-center underline"  :class="[tbIndex==tabbarIndex?'on':'']">{{grid}}</view>
			</view>
		</view>
		<!-- 考虑非APP端长列表和复杂的DOM使用scroll-view会卡顿，所以漂浮顶部选项卡使用page本身的滑动 -->
		<view class="order-list">
			<view class="list margin-lr-sm">
...
.topTabBar{
	...
	.grid{
		.underline{
			...
			&.on{
				color: #e54d42;
				border-bottom: solid 4upx #e54d42;
			}
		}
